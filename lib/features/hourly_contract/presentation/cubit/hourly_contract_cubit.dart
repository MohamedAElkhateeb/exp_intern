...
class HourlyContractCubit extends Cubit<HourlyContractState> {
  ...
  HourlyContractCubit(this._shiftRepo)
      : super(const HourlyContractState());
  ...
  Future<void> getShiftHours({
    ...
    emit(state.copyWith(
      shiftHours:
      state.shiftHours.copyWith(status: RequestStatus.loading),
    ));
  ...
  Future<void> fetchContractDurations(
      {required String serviceId}) async {
    emit(state.copyWith(
      durations:
      state.durations.copyWith(status: RequestStatus.loading),
    ));

    final result =
    await _shiftRepo.getContractDurations(serviceId: serviceId);
  ...
  Future<void> fetchNumOfVisits(
      {required String serviceId}) async {
    emit(state.copyWith(
      numOfVisits:
      state.numOfVisits.copyWith(status: RequestStatus.loading),
    ));

    final result =
    await _shiftRepo.getNumOfVisits(serviceId: serviceId);
  ...
  Future<void> fetchWorkerCounts(
      {required String serviceId}) async {
    emit(state.copyWith(
      workerCounts: state.workerCounts
          .copyWith(status: RequestStatus.loading),
    ));

    final result =
    await _shiftRepo.getWorkerCounts(serviceId: serviceId);
  ...
  Future<void> fetchTimeSlots(
      {required TimeSlotParams params}) async {
    emit(state.copyWith(
      timeSlots:
      state.timeSlots.copyWith(status: RequestStatus.loading),
    ));

    final result =
    await _shiftRepo.getTimeSlots(params: params);
  ...
  Future<void> fetchArrivalTime(
      {required String timeSlotId}) async {
    emit(state.copyWith(
      arrivalTimeStatus: RequestStatus.loading,
    ));

    final result =
    await _shiftRepo.getArrivalTime(timeSlotId: timeSlotId);

    result.fold(
          (failure) {
        emit(state.copyWith(
          arrivalTimeStatus: RequestStatus.error,
        ));
      },
          (arrivalText) {
        emit(state.copyWith(
  ...
  Future<void> fetchFixedPackages({
    ...
    result.fold(
        ...
        if (packagesResult.packageProperties is String) {
          try {
            final decoded = jsonDecode(packagesResult.packageProperties as String);
            properties = (decoded as Map<String, dynamic>).map((key, value) {
              return MapEntry(
                key,
                PackagePropertyConfigModel.fromJson(value),
              );
            });
          } catch (_) {}
        } else {
